...
function periodRows(summaries: PeriodSummary[]): string[][] {
...
function sessionRows(summaries: SessionSummary[]): string[][] {
  return summaries.map((summary) => [
    summary.day,
    summary.app,
    summary.model,
...
export function renderSingleUsageTable(
  view: UsageTableView,
  ...
  if (view === "monthly") {
    return renderTable(
      "Monthly token usage (copilot by model)",
      ["Month", "App", "Model", "Sessions", "Input", "Cached", "Output", "Reasoning", "Total"],
      periodRows(monthly)
    );
  }
  if (view === "daily") {
    return renderTable(
      "Daily token usage (copilot by model)",
      ["Day", "App", "Model", "Sessions", "Input", "Cached", "Output", "Reasoning", "Total"],
      periodRows(daily)
    );
  }
  return renderTable(
    "Session token usage (copilot by model)",
    ["Day", "App", "Model", "Session", "Input", "Cached", "Output", "Reasoning", "Total"],
