...
import { ensurePaths } from "./state.js";
...
export function resolveCopilotHome(explicitHome) {
    if (explicitHome) {
        return explicitHome;
    }
...
function usageRecordFromAssistantUsage(event, fallback) {
...
function usageRecordsFromSessionShutdown(event, fallback) {
...
function parseEventsContent(content, fallbackTimestamp, fallbackSessionId) {
    let sessionId = fallbackSessionId;
    let currentModel = "unknown";
    const directUsageRecords = [];
    const shutdownUsageRecords = [];
    for (const rawLine of content.split(/\r?\n/u)) {
        ...
        shutdownUsageRecords.push(...usageRecordsFromSessionShutdown(event, {
            sessionId,
            timestamp: fallbackTimestamp
        }));
    }
...
async function parseEventsFile(filePath, fallbackSessionId) {
    const fileStat = await stat(filePath);
    const content = await readFile(filePath, "utf8");
    return parseEventsContent(content, fileStat.mtime.toISOString(), fallbackSessionId);
}
function sessionIdFromArchiveName(fileName) {
...
async function findLatestSession(copilotHome) {
    ...
    const valid = candidates.filter((item) => Boolean(item));
...
export async function storeLatestSession(paths, copilotHomeArg) {
    ...
    await appendFile(paths.usageSummariesPath, `${JSON.stringify({
...
export async function storePromptUsageFromOutput(paths, output) {
    await ensurePaths(paths);
...
function usageRecordsFromCapturedSummary(entry, fallbackSessionId) {
...
async function parseCapturedSummaryFile(filePath) {
    const content = await readFile(filePath, "utf8");
    const records = [];
    let rowIndex = 0;
    for (const rawLine of content.split(/\r?\n/u)) {
...
export async function readStoredCopilotUsage(paths) {
    const records = [];
